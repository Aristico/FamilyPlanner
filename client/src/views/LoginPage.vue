  ...
  <div class="min-h-screen flex items-center justify-center bg-slate-100">
    <div class="max-w-md w-full bg-white p-8 rounded-lg shadow-lg">
      <div class="text-center mb-8">
        ...
        <h1 class="mt-2 text-2xl font-bold text-slate-900">Willkommen zurück!</h1>
        <p class="text-slate-500">Logge dich ein, um auf deinen Planer zuzugreifen.</p>
      </div>

      <form @submit.prevent="handleLogin">
        <div v-if="message" class="mb-4 p-3 rounded-lg text-center bg-red-100 text-red-800">
          {{ message }}
        </div>
      ...
      <div class="mt-6 text-center">
        <p class="text-sm text-slate-600">
          ...
          <router-link to="/register" class="font-medium text-primary-600 hover:text-primary-700">Hier registrieren</router-link>
        </p>
      </div>
    </div>
  </div>
...
const handleLogin = async () => {
  ...
  try {
    ...
    authStore.authenticate(result.user);

    // Redirect to the meal planner
